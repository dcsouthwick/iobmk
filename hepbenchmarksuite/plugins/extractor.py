    ...
    *********************************************************
    ...
    def __init__(self):
        ...
        for dep in req_packages:
            cmd_reply, returncode = exec_cmd("rpm -q {}".format(dep))
            if not returncode:
                _log.error(cmd_reply)  # package X is not installed
                self.pkg[dep] = False
            else:
                _log.debug("Package installed: {}".format(cmd_reply))
                self.pkg[dep] = True
    ...
    def get_parser(self, cmd_output, reg="common"):
        ...
        def parser(pattern):
            ...
            try:
                _log.debug("Parsing = {} | Field = {} | Value = {}".format(
