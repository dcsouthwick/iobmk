...
class TestSpec:
    ...
    def test_cli_interface(self, mock,  mode, bench):
        ...
        if mode == 'singularity':
            valid = 'SINGULARITY_CACHEDIR=./singularity_cachedir singularity run -B /tmp/hep-spec_wd3:/tmp/hep-spec_wd3 -B /tmp/SPEC:/tmp/SPEC docker://gitlab-registry.cern.ch/hep-benchmarks/hep-spec/hepspec-cc7-x86_64:qa  -b {} -w /tmp/hep-spec_wd3 -n None -u https://www.example.com/ -p /tmp/SPEC -i 1 -s {}'.format(bench, bmkset)

        elif mode == 'docker':
            valid = 'docker run --rm --network=host -v /tmp/hep-spec_wd3:/tmp/hep-spec_wd3:Z -v /tmp/SPEC:/tmp/SPEC:Z gitlab-registry.cern.ch/hep-benchmarks/hep-spec/hepspec-cc7-x86_64:qa  -b {} -w /tmp/hep-spec_wd3 -n None -u https://www.example.com/ -p /tmp/SPEC -i 1 -s {}'.format(bench, bmkset)
