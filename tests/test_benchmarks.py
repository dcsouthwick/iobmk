...
#         with self.assertLogs('iobenchmarksuite.benchmarks', level='INFO') as log:
...
class TestSpec:
    ...
    def test_invalid_docker_image(self):
        ...
        with cont.assertLogs('iobenchmarksuite.benchmarks', level='INFO') as log:
            assert benchmarks.run_hepspec(sample_config, 'spec2017') == 1
            cont.assertIn('ERROR:iobenchmarksuite.benchmarks:Invalid docker image specified. Image should start with docker://', " ".join(log.output))
...
#        with self.assertLogs('iobenchmarksuite.utils', level='ERROR') as log:
#            ret = utils.run_hepscore(test_conf)
#            self.assertEqual(ret, -1)
